...
<div>
  <v-app>
    <v-toolbar flat color="black">
      <v-toolbar-title class="numericite-toolbar-title">Partenaires</v-toolbar-title>
      <v-divider
        class="mx-2"
        inset
        vertical
                ...
                <v-layout wrap>
                  ...
                  <v-flex xs12>
                    <v-text-field
                      ...
                      required></v-text-field>
                    <div v-if="conflictSlug" class="custom-error">
                      Ce tag existe déjà dans notre base de donnée
                    </div>
                  </v-flex>
                  <v-flex xs12>
                    <v-autocomplete :items="schools"
                      ...
                      :error-messages="errors.collect('school_ids')"
...
<script>
  ...
  export default {
    ...
    components: { ConfirmDelete, TemplateItem },
    ...
    computed: {
      formTitle () {
        return this.editedIndex === -1 ? 'Créer un partenaire' : 'Éditer un partenaire'
      }
    },
    watch: {
      params: {
    ...
    methods: {
      ...
      async getRows() {
        ...
        return new Promise(async (resolve, reject) => {
          try {
            let request = await this.$api.partners.count()
            const count = request.data.res || 0

            request = await this.$api.partners.getList({
      ...
      async deleteItem (item) {
        try {
          await this.$api.partners.drop([item.id])
          this.getRows().then(this.bindData)
        } catch (e) {
          console.log(e)
        }
        ...
        this.$validator.validate().then(async valid => {
          if (valid && (this.tmpImg || this.editedItem.img_path)) {
            if (this.editedIndex > -1) {
              //EDIT
              try {
                await this.$api.partners.update(this.editedItem)

                if (!this.editedItem.img_path) {
                  ...
                  await this.$api.partners.uploadLogo(this.editedItem.id, file)
                }
              ...
              //CREATE
              try {
                let response = await this.$api.partners.create(this.editedItem)
                ...
                await this.$api.partners.uploadLogo(id, file)
