 ...
 */
class LeadDAOImpl @Inject() (reactiveMongoApi: ReactiveMongoApi) extends LeadDAO {

  val logger = Logger("BP." + this.getClass.getSimpleName);
  def collection: Future[JSONCollection] = reactiveMongoApi.database.map(_.collection("lead"))
  def db: Future[DefaultDB] = reactiveMongoApi.database

  import LeadFormats._
  ...
  def findAndModify(selector: BSONDocument, modifier: Lead, fields: BSONDocument): Future[Option[Lead]] = {
    collection.flatMap(_.findAndUpdate(selector, modifier, fetchNewObject = true, upsert = false).map(_.result[Lead]))
  }
  ...
  def find(selector: BSONDocument, sort: JsObject = Json.obj("updated_date" -> -1), projectionIn: BSONDocument = BSONDocument(), page: Int = 1, numberPerPage: Int = 25): Future[List[Lead]] = {
    collection.flatMap(_.find(selector)
      ...
      .sort(sort).cursor[Lead]().collect[List](numberPerPage))
  }
  ...
  def getList(page: Int, numberPerPage: Int): Future[List[Lead]] = {
    collection.flatMap(_.find(Json.obj())
      ...
      .cursor[Lead]().collect[List](numberPerPage))
  }
  ...
  def findOne(selector: BSONDocument, sort: JsObject = Json.obj("updated_date" -> -1)): Future[Option[Lead]] = {
    collection.flatMap(_.find(selector).sort(sort).one[Lead])
  }
