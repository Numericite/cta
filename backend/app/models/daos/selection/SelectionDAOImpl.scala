 ...
 */
class SelectionDAOImpl @Inject() (reactiveMongoApi: ReactiveMongoApi) extends SelectionDAO {

  val logger = Logger("BP." + this.getClass.getSimpleName);
  def collection: Future[JSONCollection] = reactiveMongoApi.database.map(_.collection("selection"))
  def db: Future[DefaultDB] = reactiveMongoApi.database

  import SelectionFormats._
  ...
  def findAndModify(selector: BSONDocument, modifier: Selection, fields: BSONDocument): Future[Option[Selection]] = {
    collection.flatMap(_.findAndUpdate(selector, modifier, fetchNewObject = true, upsert = false).map(_.result[Selection]))
  }
  ...
  def find(selector: BSONDocument, sort: JsObject = Json.obj("updated_date" -> -1), projectionIn: BSONDocument = BSONDocument(), page: Int = 1, numberPerPage: Int = 25): Future[List[Selection]] = {
    collection.flatMap(_.find(selector)
      ...
      .sort(sort).cursor[Selection]().collect[List](numberPerPage))
  }
  ...
  def getList(page: Int, numberPerPage: Int): Future[List[Selection]] = {
    collection.flatMap(_.find(Json.obj())
      ...
      .cursor[Selection]().collect[List](numberPerPage))
  }
  ...
  def findOne(selector: BSONDocument, sort: JsObject = Json.obj("updated_date" -> -1)): Future[Option[Selection]] = {
    collection.flatMap(_.find(selector).sort(sort).one[Selection])
  }
