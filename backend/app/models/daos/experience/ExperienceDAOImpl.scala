 ...
 */
class ExperienceDAOImpl @Inject() (reactiveMongoApi: ReactiveMongoApi) extends ExperienceDAO {

  val logger = Logger("BP." + this.getClass.getSimpleName);
  def collection: Future[JSONCollection] = reactiveMongoApi.database.map(_.collection("experience"))
  def db: Future[DefaultDB] = reactiveMongoApi.database

  import ExperienceFormats._
  ...
  def findAndModify(selector: BSONDocument, modifier: Experience, fields: BSONDocument): Future[Option[Experience]] = {
    collection.flatMap(_.findAndUpdate(selector, modifier, fetchNewObject = true, upsert = false).map(_.result[Experience]))
  }
  ...
  def find(selector: BSONDocument, sort: JsObject = Json.obj("updated_date" -> 1), projectionIn: BSONDocument = BSONDocument(), page: Int = 1, numberPerPage: Int = 25): Future[List[Experience]] = {
    collection.flatMap(_.find(selector)
      ...
      .sort(sort).cursor[Experience]().collect[List](numberPerPage))
  }
  ...
  def getList(page: Int, numberPerPage: Int): Future[List[Experience]] = {
    collection.flatMap(_.find(Json.obj())
      ...
      .cursor[Experience]().collect[List](numberPerPage))
  }
  ...
  def findOne(selector: BSONDocument, sort: JsObject = Json.obj("updated_date" -> 1)): Future[Option[Experience]] = {
    collection.flatMap(_.find(selector).sort(sort).one[Experience])
  }
