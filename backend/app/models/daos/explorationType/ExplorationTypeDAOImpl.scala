 ...
 */
class ExplorationTypeDAOImpl @Inject() (reactiveMongoApi: ReactiveMongoApi) extends ExplorationTypeDAO {

  val logger = Logger("BP." + this.getClass.getSimpleName);

  def collection: Future[JSONCollection] = reactiveMongoApi.database.map(_.collection("explorationType"))

  def db: Future[DefaultDB] = reactiveMongoApi.database

  import ExplorationTypeFormats._
  ...
  def findAndModify(selector: BSONDocument, modifier: ExplorationType, fields: BSONDocument): Future[Option[ExplorationType]] = {
    collection.flatMap(_.findAndUpdate(selector, modifier, fetchNewObject = true, upsert = false).map(_.result[ExplorationType]))
  }
  ...
  def find(selector: BSONDocument, sort: JsObject = Json.obj("num" -> 1), projectionIn: BSONDocument = BSONDocument(), page: Int = 1, numberPerPage: Int = 25): Future[List[ExplorationType]] = {
    collection.flatMap(_.find(selector)
      ...
      .sort(sort).cursor[ExplorationType]().collect[List](numberPerPage))
  }
  ...
  def getList(page: Int, numberPerPage: Int): Future[List[ExplorationType]] = {
    collection.flatMap(_.find(Json.obj())
      .options(QueryOpts(skipN = ((page - 1) * numberPerPage), batchSizeN = numberPerPage, flagsN = 0))
      .sort(Json.obj("num" -> 1))
      .cursor[ExplorationType]().collect[List](numberPerPage))
  }
  ...
  def findOne(selector: BSONDocument, sort: JsObject = Json.obj("num" -> 1)): Future[Option[ExplorationType]] = {
    collection.flatMap(_.find(selector).sort(sort).one[ExplorationType])
  }

  def aggregate(pipeLine: BSONArray): Future[JsObject] = {

    val commandQuery = BSONDocument(
      "aggregate" -> "explorationType",
      "pipeline" -> pipeLine
    )
