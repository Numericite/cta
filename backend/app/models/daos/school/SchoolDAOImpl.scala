 ...
 */
class SchoolDAOImpl @Inject() (reactiveMongoApi: ReactiveMongoApi) extends SchoolDAO {

  val logger = Logger("BP." + this.getClass.getSimpleName);
  def collection: Future[JSONCollection] = reactiveMongoApi.database.map(_.collection("school"))
  def db: Future[DefaultDB] = reactiveMongoApi.database

  import SchoolFormats._
  ...
  def findAndModify(selector: BSONDocument, modifier: School, fields: BSONDocument): Future[Option[School]] = {
    collection.flatMap(_.findAndUpdate(selector, modifier, fetchNewObject = true, upsert = false).map(_.result[School]))
  }
  ...
  def find(selector: BSONDocument, sort: JsObject = Json.obj("updated_date" -> -1), projectionIn: BSONDocument = BSONDocument(), page: Int = 1, numberPerPage: Int = 25): Future[List[School]] = {
    collection.flatMap(_.find(selector)
      ...
      .sort(sort).cursor[School]().collect[List](numberPerPage))
  }
  ...
  def getList(page: Int, numberPerPage: Int): Future[List[School]] = {
    collection.flatMap(_.find(Json.obj())
      ...
      .cursor[School]().collect[List](numberPerPage))
  }
  ...
  def findOne(selector: BSONDocument, sort: JsObject = Json.obj("updated_date" -> -1)): Future[Option[School]] = {
    collection.flatMap(_.find(selector).sort(sort).one[School])
  }
