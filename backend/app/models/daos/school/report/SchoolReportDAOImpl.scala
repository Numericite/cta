 ...
 */
class SchoolReportDAOImpl @Inject() (reactiveMongoApi: ReactiveMongoApi) extends SchoolReportDAO {

  val logger = Logger("BP." + this.getClass.getSimpleName);
  def collection: Future[JSONCollection] = reactiveMongoApi.database.map(_.collection("schoolReport"))
  def db: Future[DefaultDB] = reactiveMongoApi.database

  import SchoolReportFormats._
  ...
  def findAndModify(selector: BSONDocument, modifier: SchoolReport, fields: BSONDocument): Future[Option[SchoolReport]] = {
    collection.flatMap(_.findAndUpdate(selector, modifier, fetchNewObject = true, upsert = false).map(_.result[SchoolReport]))
  }
  ...
  def find(selector: BSONDocument, sort: JsObject = Json.obj("updated_date" -> -1), projectionIn: BSONDocument = BSONDocument(), page: Int = 1, numberPerPage: Int = 25): Future[List[SchoolReport]] = {
    collection.flatMap(_.find(selector)
      ...
      .sort(sort).cursor[SchoolReport]().collect[List](numberPerPage))
  }
  ...
  def getList(page: Int, numberPerPage: Int): Future[List[SchoolReport]] = {
    collection.flatMap(_.find(Json.obj())
      ...
      .cursor[SchoolReport]().collect[List](numberPerPage))
  }
  ...
  def findOne(selector: BSONDocument, sort: JsObject = Json.obj("updated_date" -> -1)): Future[Option[SchoolReport]] = {
    collection.flatMap(_.find(selector).sort(sort).one[SchoolReport])
  }

  def aggregate(pipeLine: BSONArray): Future[JsObject] = {

    val commandQuery = BSONDocument(
      "aggregate" -> "schoolReport",
      "pipeline" -> pipeLine
    )
