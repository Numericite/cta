 ...
 */
class InternshipDAOImpl @Inject() (reactiveMongoApi: ReactiveMongoApi) extends InternshipDAO {

  val logger = Logger("BP." + this.getClass.getSimpleName);
  def collection: Future[JSONCollection] = reactiveMongoApi.database.map(_.collection("internship"))
  def db: Future[DefaultDB] = reactiveMongoApi.database

  import InternshipFormats._
  ...
  def findAndModify(selector: BSONDocument, modifier: Internship, fields: BSONDocument): Future[Option[Internship]] = {
    collection.flatMap(_.findAndUpdate(selector, modifier, fetchNewObject = true, upsert = false).map(_.result[Internship]))
  }
  ...
  def find(selector: BSONDocument, sort: JsObject = Json.obj("updated_date" -> -1), projectionIn: BSONDocument = BSONDocument(), page: Int = 1, numberPerPage: Int = 25): Future[List[Internship]] = {
    collection.flatMap(_.find(selector)
      ...
      .sort(sort).cursor[Internship]().collect[List](numberPerPage))
  }
  ...
  def getList(page: Int, numberPerPage: Int): Future[List[Internship]] = {
    collection.flatMap(_.find(Json.obj())
      ...
      .cursor[Internship]().collect[List](numberPerPage))
  }
  ...
  def findOne(selector: BSONDocument, sort: JsObject = Json.obj("updated_date" -> -1)): Future[Option[Internship]] = {
    collection.flatMap(_.find(selector).sort(sort).one[Internship])
  }
