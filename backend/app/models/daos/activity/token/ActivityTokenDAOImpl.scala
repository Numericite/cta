 ...
 */
class ActivityTokenDAOImpl @Inject() (reactiveMongoApi: ReactiveMongoApi) extends ActivityTokenDAO {

  val logger = Logger("BP." + this.getClass.getSimpleName);
  def collection: Future[JSONCollection] = reactiveMongoApi.database.map(_.collection("activityToken"))
  def db: Future[DefaultDB] = reactiveMongoApi.database

  import ActivityTokenFormats._
  ...
  def findAndModify(selector: BSONDocument, modifier: ActivityToken, fields: BSONDocument): Future[Option[ActivityToken]] = {
    collection.flatMap(_.findAndUpdate(selector, modifier, fetchNewObject = true, upsert = false).map(_.result[ActivityToken]))
  }
  ...
  def find(selector: BSONDocument, sort: JsObject = Json.obj("created_date" -> -1), projectionIn: BSONDocument = BSONDocument(), page: Int = 1, numberPerPage: Int = 25): Future[List[ActivityToken]] = {
    collection.flatMap(_.find(selector)
      ...
      .sort(sort).cursor[ActivityToken]().collect[List](numberPerPage))
  }
  ...
  def getList(page: Int, numberPerPage: Int): Future[List[ActivityToken]] = {
    collection.flatMap(_.find(Json.obj())
      ...
      .cursor[ActivityToken]().collect[List](numberPerPage))
  }
  ...
  def findOne(selector: BSONDocument, sort: JsObject = Json.obj("created_date" -> -1)): Future[Option[ActivityToken]] = {
    collection.flatMap(_.find(selector).sort(sort).one[ActivityToken])
  }

  def aggregate(pipeLine: BSONArray): Future[JsObject] = {

    val commandQuery = BSONDocument(
      "aggregate" -> "activityToken",
      "pipeline" -> pipeLine
    )
