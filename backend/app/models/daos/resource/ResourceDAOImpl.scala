 ...
 */
class ResourceDAOImpl @Inject() (reactiveMongoApi: ReactiveMongoApi) extends ResourceDAO {

  val logger = Logger("BP." + this.getClass.getSimpleName);
  def collection: Future[JSONCollection] = reactiveMongoApi.database.map(_.collection("resource"))
  def db: Future[DefaultDB] = reactiveMongoApi.database

  import ResourceFormats._
  ...
  def findAndModify(selector: BSONDocument, modifier: Resource, fields: BSONDocument): Future[Option[Resource]] = {
    collection.flatMap(_.findAndUpdate(selector, modifier, fetchNewObject = true, upsert = false).map(_.result[Resource]))
  }
  ...
  def find(selector: BSONDocument, sort: JsObject = Json.obj("updated_date" -> -1), projectionIn: BSONDocument = BSONDocument(), page: Int = 1, numberPerPage: Int = 25): Future[List[Resource]] = {
    collection.flatMap(_.find(selector)
      ...
      .sort(sort).cursor[Resource]().collect[List](numberPerPage))
  }
  ...
  def getList(page: Int, numberPerPage: Int): Future[List[Resource]] = {
    collection.flatMap(_.find(Json.obj())
      ...
      .cursor[Resource]().collect[List](numberPerPage))
  }
  ...
  def findOne(selector: BSONDocument, sort: JsObject = Json.obj("updated_date" -> -1)): Future[Option[Resource]] = {
    collection.flatMap(_.find(selector).sort(sort).one[Resource])
  }
