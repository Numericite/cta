 ...
 */
class AssociationDAOImpl @Inject() (reactiveMongoApi: ReactiveMongoApi) extends AssociationDAO {

  val logger = Logger("BP." + this.getClass.getSimpleName);
  def collection: Future[JSONCollection] = reactiveMongoApi.database.map(_.collection("association"))
  def db: Future[DefaultDB] = reactiveMongoApi.database

  import AssociationFormats._
  ...
  def findAndModify(selector: BSONDocument, modifier: Association, fields: BSONDocument): Future[Option[Association]] = {
    collection.flatMap(_.findAndUpdate(selector, modifier, fetchNewObject = true, upsert = false).map(_.result[Association]))
  }
  ...
  def find(selector: BSONDocument, sort: JsObject = Json.obj("updated_date" -> -1), projectionIn: BSONDocument = BSONDocument(), page: Int = 1, numberPerPage: Int = 25): Future[List[Association]] = {
    collection.flatMap(_.find(selector)
      ...
      .sort(sort).cursor[Association]().collect[List](numberPerPage))
  }
  ...
  def getList(page: Int, numberPerPage: Int): Future[List[Association]] = {
    collection.flatMap(_.find(Json.obj())
      ...
      .cursor[Association]().collect[List](numberPerPage))
  }
  ...
  def findOne(selector: BSONDocument, sort: JsObject = Json.obj("updated_date" -> -1)): Future[Option[Association]] = {
    collection.flatMap(_.find(selector).sort(sort).one[Association])
  }
