 ...
 */
class DetailDAOImpl @Inject() (reactiveMongoApi: ReactiveMongoApi) extends DetailDAO {

  val logger = Logger("BP." + this.getClass.getSimpleName);
  def collection: Future[JSONCollection] = reactiveMongoApi.database.map(_.collection("detail"))
  def db: Future[DefaultDB] = reactiveMongoApi.database

  import DetailFormats._
  ...
  def findAndModify(selector: BSONDocument, modifier: Detail, fields: BSONDocument): Future[Option[Detail]] = {
    collection.flatMap(_.findAndUpdate(selector, modifier, fetchNewObject = true, upsert = false).map(_.result[Detail]))
  }
  ...
  def find(selector: BSONDocument, sort: JsObject = Json.obj("updated_date" -> -1), projectionIn: BSONDocument = BSONDocument(), page: Int = 1, numberPerPage: Int = 25): Future[List[Detail]] = {
    collection.flatMap(_.find(selector)
      ...
      .sort(sort).cursor[Detail]().collect[List](numberPerPage))
  }
  ...
  def getList(page: Int, numberPerPage: Int): Future[List[Detail]] = {
    collection.flatMap(_.find(Json.obj())
      ...
      .cursor[Detail]().collect[List](numberPerPage))
  }
  ...
  def findOne(selector: BSONDocument, sort: JsObject = Json.obj("updated_date" -> -1)): Future[Option[Detail]] = {
    collection.flatMap(_.find(selector).sort(sort).one[Detail])
  }
