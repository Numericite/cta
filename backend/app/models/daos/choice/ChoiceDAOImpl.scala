 ...
 */
class ChoiceDAOImpl @Inject() (reactiveMongoApi: ReactiveMongoApi) extends ChoiceDAO {

  val logger = Logger("BP." + this.getClass.getSimpleName);
  def collection: Future[JSONCollection] = reactiveMongoApi.database.map(_.collection("choice"))
  def db: Future[DefaultDB] = reactiveMongoApi.database

  import ChoiceFormats._
  ...
  def findAndModify(selector: BSONDocument, modifier: Choice, fields: BSONDocument): Future[Option[Choice]] = {
    collection.flatMap(_.findAndUpdate(selector, modifier, fetchNewObject = true, upsert = false).map(_.result[Choice]))
  }
  ...
  def find(selector: BSONDocument, sort: JsObject = Json.obj("updated_date" -> -1), projectionIn: BSONDocument = BSONDocument(), page: Int = 1, numberPerPage: Int = 25): Future[List[Choice]] = {
    collection.flatMap(_.find(selector)
      ...
      .sort(sort).cursor[Choice]().collect[List](numberPerPage))
  }
  ...
  def getList(page: Int, numberPerPage: Int): Future[List[Choice]] = {
    collection.flatMap(_.find(Json.obj())
      ...
      .cursor[Choice]().collect[List](numberPerPage))
  }
  ...
  def findOne(selector: BSONDocument, sort: JsObject = Json.obj("updated_date" -> -1)): Future[Option[Choice]] = {
    collection.flatMap(_.find(selector).sort(sort).one[Choice])
  }
