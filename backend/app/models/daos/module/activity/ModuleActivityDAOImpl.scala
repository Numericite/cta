 ...
 */
class ModuleActivityDAOImpl @Inject() (reactiveMongoApi: ReactiveMongoApi) extends ModuleActivityDAO {

  val logger = Logger("BP." + this.getClass.getSimpleName);
  def collection: Future[JSONCollection] = reactiveMongoApi.database.map(_.collection("moduleActivity"))
  ...
  def findAndModify(selector: BSONDocument, modifier: ModuleActivity, fields: BSONDocument): Future[Option[ModuleActivity]] = {
    collection.flatMap(_.findAndUpdate(selector, modifier, fetchNewObject = true, upsert = false).map(_.result[ModuleActivity]))
  }
  ...
  def find(selector: BSONDocument, sort: JsObject = Json.obj("num" -> 1), projectionIn: BSONDocument = BSONDocument(), page: Int = 1, numberPerPage: Int = 25): Future[List[ModuleActivity]] = {
    collection.flatMap(_.find(selector)
      ...
      .sort(sort).cursor[ModuleActivity]().collect[List](numberPerPage))
  }
  ...
  def getList(page: Int, numberPerPage: Int): Future[List[ModuleActivity]] = {
    collection.flatMap(_.find(Json.obj())
      .options(QueryOpts(skipN = ((page - 1) * numberPerPage), batchSizeN = numberPerPage, flagsN = 0))
      .sort(Json.obj("num" -> 1))
      .cursor[ModuleActivity]().collect[List](numberPerPage))
  }
  ...
  def findOne(selector: BSONDocument, sort: JsObject = Json.obj("num" -> 1)): Future[Option[ModuleActivity]] = {
    collection.flatMap(_.find(selector).sort(sort).one[ModuleActivity])
  }

  def aggregate(pipeLine: BSONArray): Future[JsObject] = {

    val commandQuery = BSONDocument(
      "aggregate" -> "moduleActivity",
      "pipeline" -> pipeLine
    )
