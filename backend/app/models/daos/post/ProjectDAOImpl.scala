 ...
 */
class PostDAOImpl @Inject() (reactiveMongoApi: ReactiveMongoApi) extends PostDAO {

  val logger = Logger("BP." + this.getClass.getSimpleName);
  def collection: Future[JSONCollection] = reactiveMongoApi.database.map(_.collection("post"))
  def db: Future[DefaultDB] = reactiveMongoApi.database

  import PostFormats._
  ...
  def findAndModify(selector: BSONDocument, modifier: Post, fields: BSONDocument): Future[Option[Post]] = {
    collection.flatMap(_.findAndUpdate(selector, modifier, fetchNewObject = true, upsert = false).map(_.result[Post]))
  }
  ...
  def find(selector: BSONDocument, sort: JsObject = Json.obj("updated_date" -> -1), projectionIn: BSONDocument = BSONDocument(), page: Int = 1, numberPerPage: Int = 25): Future[List[Post]] = {
    collection.flatMap(_.find(selector)
      ...
      .sort(sort).cursor[Post]().collect[List](numberPerPage))
  }
  ...
  def getList(page: Int, numberPerPage: Int): Future[List[Post]] = {
    collection.flatMap(_.find(Json.obj())
      ...
      .cursor[Post]().collect[List](numberPerPage))
  }
  ...
  def findOne(selector: BSONDocument, sort: JsObject = Json.obj("updated_date" -> -1)): Future[Option[Post]] = {
    collection.flatMap(_.find(selector).sort(sort).one[Post])
  }
