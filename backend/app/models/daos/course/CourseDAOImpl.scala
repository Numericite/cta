 ...
 */
class CourseDAOImpl @Inject() (reactiveMongoApi: ReactiveMongoApi) extends CourseDAO {

  val logger = Logger("BP." + this.getClass.getSimpleName);
  def collection: Future[JSONCollection] = reactiveMongoApi.database.map(_.collection("course"))
  def db: Future[DefaultDB] = reactiveMongoApi.database

  import CourseFormats._
  ...
  def findAndModify(selector: BSONDocument, modifier: Course, fields: BSONDocument): Future[Option[Course]] = {
    collection.flatMap(_.findAndUpdate(selector, modifier, fetchNewObject = true, upsert = false).map(_.result[Course]))
  }
  ...
  def find(selector: BSONDocument, sort: JsObject = Json.obj("updated_date" -> -1), projectionIn: BSONDocument = BSONDocument(), page: Int = 1, numberPerPage: Int = 25): Future[List[Course]] = {
    collection.flatMap(_.find(selector)
      ...
      .sort(sort).cursor[Course]().collect[List](numberPerPage))
  }
  ...
  def getList(page: Int, numberPerPage: Int): Future[List[Course]] = {
    collection.flatMap(_.find(Json.obj())
      ...
      .cursor[Course]().collect[List](numberPerPage))
  }
  ...
  def findOne(selector: BSONDocument, sort: JsObject = Json.obj("updated_date" -> -1)): Future[Option[Course]] = {
    collection.flatMap(_.find(selector).sort(sort).one[Course])
  }
