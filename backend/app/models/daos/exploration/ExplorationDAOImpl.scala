 ...
 */
class ExplorationDAOImpl @Inject() (reactiveMongoApi: ReactiveMongoApi) extends ExplorationDAO {

  val logger = Logger("BP." + this.getClass.getSimpleName);

  def collection: Future[JSONCollection] = reactiveMongoApi.database.map(_.collection("exploration"))

  def db: Future[DefaultDB] = reactiveMongoApi.database

  import ExplorationFormats._
  ...
  def findAndModify(selector: BSONDocument, modifier: Exploration, fields: BSONDocument): Future[Option[Exploration]] = {
    collection.flatMap(_.findAndUpdate(selector, modifier, fetchNewObject = true, upsert = false).map(_.result[Exploration]))
  }
  ...
  def find(selector: BSONDocument, sort: JsObject = Json.obj("num" -> 1), projectionIn: BSONDocument = BSONDocument(), page: Int = 1, numberPerPage: Int = 25): Future[List[Exploration]] = {
    collection.flatMap(_.find(selector)
      ...
      .sort(sort).cursor[Exploration]().collect[List](numberPerPage))
  }
  ...
  def getList(page: Int, numberPerPage: Int): Future[List[Exploration]] = {
    collection.flatMap(_.find(Json.obj())
      .options(QueryOpts(skipN = ((page - 1) * numberPerPage), batchSizeN = numberPerPage, flagsN = 0))
      .sort(Json.obj("num" -> 1))
      .cursor[Exploration]().collect[List](numberPerPage))
  }
  ...
  def findOne(selector: BSONDocument, sort: JsObject = Json.obj("num" -> 1)): Future[Option[Exploration]] = {
    collection.flatMap(_.find(selector).sort(sort).one[Exploration])
  }
