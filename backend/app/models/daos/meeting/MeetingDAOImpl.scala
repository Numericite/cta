 ...
 */
class MeetingDAOImpl @Inject() (reactiveMongoApi: ReactiveMongoApi) extends MeetingDAO {

  val logger = Logger("BP." + this.getClass.getSimpleName);
  def collection: Future[JSONCollection] = reactiveMongoApi.database.map(_.collection("meeting"))
  def db: Future[DefaultDB] = reactiveMongoApi.database

  import MeetingFormats._
  ...
  def findAndModify(selector: BSONDocument, modifier: Meeting, fields: BSONDocument): Future[Option[Meeting]] = {
    collection.flatMap(_.findAndUpdate(selector, modifier, fetchNewObject = true, upsert = false).map(_.result[Meeting]))
  }
  ...
  def find(selector: BSONDocument, sort: JsObject = Json.obj("updated_date" -> -1), projectionIn: BSONDocument = BSONDocument(), page: Int = 1, numberPerPage: Int = 25): Future[List[Meeting]] = {
    collection.flatMap(_.find(selector)
      ...
      .sort(sort).cursor[Meeting]().collect[List](numberPerPage))
  }
  ...
  def getList(page: Int, numberPerPage: Int): Future[List[Meeting]] = {
    collection.flatMap(_.find(Json.obj())
      ...
      .cursor[Meeting]().collect[List](numberPerPage))
  }
  ...
  def findOne(selector: BSONDocument, sort: JsObject = Json.obj("updated_date" -> -1)): Future[Option[Meeting]] = {
    collection.flatMap(_.find(selector).sort(sort).one[Meeting])
  }
