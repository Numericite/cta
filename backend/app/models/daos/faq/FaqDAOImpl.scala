 ...
 */
class FaqDAOImpl @Inject() (reactiveMongoApi: ReactiveMongoApi) extends FaqDAO {

  val logger = Logger("BP." + this.getClass.getSimpleName);
  def collection: Future[JSONCollection] = reactiveMongoApi.database.map(_.collection("faq"))
  ...
  def findAndModify(selector: BSONDocument, modifier: Faq, fields: BSONDocument): Future[Option[Faq]] = {
    collection.flatMap(_.findAndUpdate(selector, modifier, fetchNewObject = true, upsert = false).map(_.result[Faq]))
  }
  ...
  def find(selector: BSONDocument, sort: JsObject = Json.obj("updated_date" -> -1), projectionIn: BSONDocument = BSONDocument(), page: Int = 1, numberPerPage: Int = 25): Future[List[Faq]] = {
    collection.flatMap(_.find(selector)
      ...
      .sort(sort).cursor[Faq]().collect[List](numberPerPage))
  }
  ...
  def getList(page: Int, numberPerPage: Int): Future[List[Faq]] = {
    collection.flatMap(_.find(Json.obj())
      ...
      .cursor[Faq]().collect[List](numberPerPage))
  }
  ...
  def findOne(selector: BSONDocument, sort: JsObject = Json.obj("updated_date" -> -1)): Future[Option[Faq]] = {
    collection.flatMap(_.find(selector).sort(sort).one[Faq])
  }
