 ...
 */
class VersionDAOImpl @Inject() (reactiveMongoApi: ReactiveMongoApi) extends VersionDAO {

  val logger = Logger("BP." + this.getClass.getSimpleName);
  def collection: Future[JSONCollection] = reactiveMongoApi.database.map(_.collection("version"))
  def db: Future[DefaultDB] = reactiveMongoApi.database

  import VersionFormats._
  ...
  def findAndModify(selector: BSONDocument, modifier: Version, fields: BSONDocument): Future[Option[Version]] = {
    collection.flatMap(_.findAndUpdate(selector, modifier, fetchNewObject = true, upsert = false).map(_.result[Version]))
  }
  ...
  def find(selector: BSONDocument, sort: JsObject = Json.obj("updated_date" -> -1), projectionIn: BSONDocument = BSONDocument(), page: Int = 1, numberPerPage: Int = 25): Future[List[Version]] = {
    collection.flatMap(_.find(selector)
      ...
      .sort(sort).cursor[Version]().collect[List](numberPerPage))
  }
  ...
  def getList(page: Int, numberPerPage: Int): Future[List[Version]] = {
    collection.flatMap(_.find(Json.obj())
      ...
      .cursor[Version]().collect[List](numberPerPage))
  }
  ...
  def findOne(selector: BSONDocument, sort: JsObject = Json.obj("updated_date" -> -1)): Future[Option[Version]] = {
    collection.flatMap(_.find(selector).sort(sort).one[Version])
  }
