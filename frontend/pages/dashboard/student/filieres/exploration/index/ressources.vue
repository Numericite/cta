...
<script>
  ...
  export default {
    ...
    methods: {
      ...
      async getRows() {
        try {
          let response = await this.$api.resources.count({parent_type: this.$store.state.auth.user.config.accountType + '-stream'})
          this.count = response.data.res || 0

          this.pagination.numberOfPages = Math.trunc(this.count / this.pagination.numberPerPage) + 1

          response = await this.$api.resources.getList({parent_type: this.$store.state.auth.user.config.accountType + '-stream', page: this.pagination.page, numberPerPage: this.pagination.numberPerPage})
