...
<script>
  ...
  export default {
    ...
    methods: {
      ...
      async getRows() {
        try {
          let response = await this.$api.documents.count({parent_type: this.$store.state.auth.user.config.accountType + '-domain'})
          this.count = response.data.res || 0
          this.pagination.numberOfPages = Math.trunc(this.count / this.pagination.numberPerPage)

          response = await this.$api.documents.getList({parent_type: this.$store.state.auth.user.config.accountType + '-domain', page: this.pagination.page, numberPerPage: this.pagination.numberPerPage})
