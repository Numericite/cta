<template>
  <div class="activity w-screen xl:h-screen flex justify-end items-start sm:pb-16">
    ...
    <nuxt-child :competences="competences"
                :results="results"
...
export default {
  ...
  async asyncData( { app, store, error } ) {
    try {
      ...
      if(!_.some(introspection.activities, {'id': '998958fe-582e-4130-b354-bec2bcd3bbf8'})) {
        return error({ statusCode: 404, message: 'Post not found' })
      }

      let response
      const version_id = _.get(_.find(introspection.activities, {id: '998958fe-582e-4130-b354-bec2bcd3bbf8'}), 'version_id', null)

      if (version_id) {
        response = await app.$api.activities.getVersionsByIds([version_id])
      } else {
        response = await app.$api.activities.getVersions({
          parent_ids: ['998958fe-582e-4130-b354-bec2bcd3bbf8'],
          isDefault: true
        })
      }
      ...
      const selections_ids = competences.map( selection => selection.id )
      ...
      competences.forEach(function(competence) {
        competence.actions = _.filter(actions, {selection_id: competence.id})
      })

      response = await app.$api.activities.getUserLogs(app.store.state.auth.user.userID, '998958fe-582e-4130-b354-bec2bcd3bbf8')
      const activityLogs = response.data[0]

      if (activityLogs) {
        return { competences, activityLogs }
      } else {
        return { competences }
      }
    } catch ( e ) {
  ...
  data() {
    return {
      ...
      activityLogs: {
        status_name: 'open',
        activity_id: '998958fe-582e-4130-b354-bec2bcd3bbf8',
        config: {}
      },
      endResults: null
    }
  ...
  methods: {
    startActivity: function() {
      this.step = 1
      this.$router.push( 'competences/etape-1' )
    },
