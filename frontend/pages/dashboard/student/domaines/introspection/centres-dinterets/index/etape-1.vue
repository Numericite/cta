<template>
  <div class="activity__right min-h-screen relative bg-blue-lightest xl:w-3/5 w-full pb-12 pt-6 xl:pl-16 px-6 xl:pr-32 flex flex-col justify-center h-auto">
    ...
    <div class="text-blue font-bold text-xl leading-normal mt-6 mb-4">
      <span v-html="texts[1]" />
    </div>
    <!-- End Title -->
    <div class="flex flex-wrap">
      <div v-for="(centre, key) in centres"
           :key="key"
           class="p-4 w-1/3 sm:w-full flex flex-grow"
           @click="centre.like = (!allChecked ? !centre.like : false)">

        <div :class="allChecked && !centre.like ? 'disabled' : 'cursor-pointer'"
             class="card cardShadow centre rounded-xlg w-full p-6">
          <div class="flex h-6 mb-2">
            <span :class="allChecked && !centre.like ? 'text-blue-lighter' : 'text-blue'"
                  class="w-3/4 uppercase text-blue font-bold text-lg">{{ centre.text }}</span>
            <span class="w-1/4 text-right heart">
              <svg :class="allChecked && !centre.like ? 'text-blue-lighter' : 'text-orange'"
                   xmlns="http://www.w3.org/2000/svg"
                   width="11"
                   height="10"
                   viewBox="0 0 11 10"
                   class=" h-5 w-5 fill-current">
                <path v-if="centre.like"
          ...
          <div class="desc flex flex-wrap text-blue-lighter text-xs">
            <span class="leading-normal truncate" v-html="centre.displayDescription"/>
          </div>
        </div>
      </div>
    </div>
  ...
  mounted() {
    ...
    if (this.activityLogs.selection_ids) {
      ...
      this.centres.forEach(function(centre) {
        centre.like = selection_ids.indexOf(centre.id) !== -1
      })
    }
  },
  methods: {
    nextPage() {
      const results = this.centres.filter(function(centre) {
        return centre.like
      })
      ...
      this.$router.push( '/dashboard/student/domaines/introspection/centres-dinterets/etape-2' )
    }
  }
}
</script>
...
.activity {
  ...
  .card {
    ...
    &:not(.disabled) {
      &:hover {
        box-shadow: 0 8px 25px 10px rgba(14, 7, 51, 0.1);
      }
    }
  }
  .heart {
