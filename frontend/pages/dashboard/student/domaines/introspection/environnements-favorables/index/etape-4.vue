<template>
  <div class="activity__right relative min-h-screen bg-blue-lightest xl:w-3/5 w-full pb-12 pt-6 xl:pl-16 px-6 xl:pr-32 flex flex-col justify-center h-auto">
    ...
    <!-- Title -->
    <div class="mt-6 flex">
      <span class="text-blue-lighter font-bold text-6xl mr-4 sm:text-3xl">4.</span>
      <h1 class="title text-blue font-bold text-3xl leading-normal xl:mt-4 sm:text-xl">
        Travailles-tu dans un espace avec des gens ou plutôt seul(e) ?
      </h1>
    </div>
    <!-- End Title -->
...
export default {
  ...
  created() {
    this.choices = this.actions.filter(function(action) {
      return action.selection_id === '4c0c813f-e655-4128-b508-6c2127e2c244'
    })
  ...
  methods: {
    nextPage() {
      ...
      this.$router.push( '/dashboard/student/domaines/introspection/environnements-favorables/etape-5' )
    }
  }
}
</script>
