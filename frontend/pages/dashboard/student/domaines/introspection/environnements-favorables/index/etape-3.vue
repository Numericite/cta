<template>
  <div class="activity__right min-h-screen relative bg-blue-lightest xl:w-3/5 w-full pb-12 pt-6 xl:pl-16 px-6 xl:pr-32 flex flex-col justify-center h-auto">
    ...
    <!-- Title -->
    <div class="mt-6 flex">
      <span class="text-blue-lighter font-bold text-6xl mr-4 sm:text-3xl">3.</span>
      <h1 class="title text-blue font-bold text-3xl leading-normal xl:mt-4 sm:text-xl">
        Es-tu plutôt dans un bureau ou à l'extérieur ?
      </h1>
    </div>
    <!-- End Title -->
...
export default {
  ...
  created() {
    this.choices = this.actions.filter(function(action) {
      return action.selection_id === 'c1570dc0-22db-4b33-8f3c-bcbeb6606276'
    })
  ...
  methods: {
    nextPage() {
      ...
      this.$router.push( '/dashboard/student/domaines/introspection/environnements-favorables/etape-4' )
    }
  }
}
</script>
