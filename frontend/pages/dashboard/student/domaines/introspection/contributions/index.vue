      ...
      <div v-else
        ...
        <picture class="absolute pin-b pin-r brainBgBottom">
          <img src="~/assets/img/competences/bg-bottom-right.svg"
               alt="background mes contributions">
        </picture>
      </div>
      <!-- End Step 1 -->
...
export default {
  ...
  async asyncData( { app, store, error } ) {
    try {
      ...
      if(!_.some(introspection.activities, {'id': 'ad6317e1-e6bb-4449-82bb-c996671e7011'})) {
        return error({ statusCode: 404, message: 'Post not found' })
      }

      let response
      const version_id = _.get(_.find(introspection.activities, {id: 'ad6317e1-e6bb-4449-82bb-c996671e7011'}), 'version_id', null)

      if (version_id) {
        response = await app.$api.activities.getVersionsByIds([version_id])
      } else {
        response = await app.$api.activities.getVersions({
          parent_ids: ['ad6317e1-e6bb-4449-82bb-c996671e7011'],
          isDefault: true
        })
      }
      ...
      const selections_ids = missions.map( selection => selection.id )
      ...
      missions.forEach(function(mission) {
        mission.like = false
        mission.actions = actions.filter(function(action) {
          return action.selection_id === mission.id
        })
      })

      response = await app.$api.activities.getUserLogs(app.store.state.auth.user.userID, 'ad6317e1-e6bb-4449-82bb-c996671e7011')
      const activityLogs = response.data[0]

      if (activityLogs) {
        return { missions, actions, activityLogs }
      } else {
  ...
  data() {
    return {
      ...
      activityLogs: {
        status_name: 'open',
        activity_id: 'ad6317e1-e6bb-4449-82bb-c996671e7011',
        config: {}
      },
      endResults: null
    }
  ...
  methods: {
    startActivity: function() {
      this.step = 1
      this.$router.push( 'contributions/etape-1' )
    },
