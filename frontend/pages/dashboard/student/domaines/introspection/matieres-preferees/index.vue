  ...
  <div class="activity w-screen xl:min-h-screen flex justify-end items-start sm:pb-16">
    ...
    <div :class="[step > 0 ? 'bg-blue' : 'bg-home']"
      ...
      <picture v-if="step == 0"
        ...
        <img src="~/assets/img/activity-1.png"
      ...
      <div v-else
        ...
        <picture class="max-w-full absolute pin-t pin-l sm:w-1/3">
          <img src="~/assets/img/matieres-preferees/bg-top-left.svg"
               alt="background intelligences multiples">
        </picture>

        <picture v-if="step != 0"
                 class="h-full w-full flex items-center justify-center sm:h-16 sm:w-1/4 sm:bg-blue sm:rounded-full brain sm:relative">
          <svg xmlns="http://www.w3.org/2000/svg"
            ...
            <defs>
              ...
              <path id="c"
                    d="M0 .982h230.893V127.58H0z" />
              <path id="e"
                    d="M0 0h271.275v179.314H0z" />
            </defs>
            <g fill="none"
               fill-rule="evenodd">
              <path fill="#281586"
                    d="M19.734 260.091c3.92 10.35 10.427 19.036 19.887 26.556 9.219 7.326 19.727 15.138 34.202 23.097-17.816-14.662-35.134-31.734-39.087-42.545-.106-.015-.21-.024-.319-.04-.296-.599-.609-1.18-.874-1.81-1.696-4.035-3.481-8.948-6.442-12.495-4.319-6.405-12.733-12.171-18.274-3.698-.576.88-.455 1.697.006 2.307 1.525.105 3.068.514 4.517 1.252 2.909 1.482 5.117 4.034 6.384 7.376" />
              <path fill="#281586"
                    d="M91.91 318.737c-3.515-4.543-7.218-9.752-9.419-11.738.142-.193-13.994-17.387-23.401-19.176-.39-.074-.773-.024-1.139.083-10.08-17.415-11.885-19.176-17.947-21.127-.242-.105-.71-.11-1.264-.062 4.888 11.788 29.21 34.534 52.937 51.919.075.03.156.066.234.1" />
              ...
              <path fill="#106"
                    fill-rule="nonzero"
                    d="M126.876 446.392c15.763-16.432 31.678-22.234 47.745-17.406-7.494 14.998-23.409 20.8-47.745 17.406z" />
              <path fill="#281586"
                    d="M425.266 260.091c-3.92 10.35-10.427 19.036-19.887 26.556-9.219 7.326-19.727 15.138-34.202 23.097 17.816-14.662 35.134-31.734 39.087-42.545.106-.015.21-.024.319-.04.296-.599.609-1.18.874-1.81 1.696-4.035 3.481-8.948 6.442-12.495 4.319-6.405 12.733-12.171 18.274-3.698.576.88.455 1.697-.006 2.307a11.733 11.733 0 0 0-4.517 1.252c-2.909 1.482-5.117 4.034-6.384 7.376" />
              <path fill="#281586"
                    d="M353.09 318.737c3.515-4.543 7.218-9.752 9.419-11.738-.142-.193 13.994-17.387 23.401-19.176.39-.074.773-.024 1.139.083 10.08-17.415 11.885-19.176 17.947-21.127.242-.105.71-.11 1.264-.062-4.888 11.788-29.21 34.534-52.937 51.919-.075.03-.156.066-.234.1" />
              ...
              <path fill="#106"
                    fill-rule="nonzero"
                    d="M270.409 428.987c15.762 16.432 31.677 22.233 47.744 17.405-7.494-14.997-23.408-20.8-47.744-17.405z" />
              <g v-if="step > 4"
                 stroke-width="2">
                <path stroke="#A2ACE3"
                      stroke-linecap="round"
                      d="M186 9a8 8 0 1 1-16 0 8 8 0 0 1 16 0z" />
                <path stroke="#FFC600"
                      stroke-linecap="round"
                      d="M185 318.5a5.5 5.5 0 1 1-11 0 5.5 5.5 0 1 1 11 0z" />
                <g stroke="#A2ACE3"
                   stroke-linecap="round">
                ...
                <path stroke="#FF9C9C"
                      d="M14.963 198.176L2 192.889 7.664 179l12.964 5.287z" />
                <g stroke="#A2ACE3"
                   stroke-linecap="round">
                ...
                <g stroke-linecap="round">
                  <path stroke="#1DD4B6"
                        d="M216.994 52.479v39" />
                  <path stroke="#A2ACE3"
                        d="M223.994 36.479v22" />
                </g>
                <g stroke-linecap="round">
                  <path stroke="#A2ACE3"
                        d="M379.995 178.479l23-12" />
                  <path stroke="#FFC600"
                        d="M398.995 178.479l18-10" />
                </g>
              </g>
        ...
        <picture class="absolute pin-b pin-r brainBgBottom">
          <img src="~/assets/img/matieres-preferees/bg-bottom-right.svg"
               alt="background intelligences multiples">
        </picture>
      </div>
      <!-- End Step 1 -->
...
export default {
  ...
  async asyncData( { app, route, error, store } ) {
    try {
      ...
      if(!_.some(introspection.activities, {'id': 'eac4491c-9d01-474d-b5c0-3bfa095d2b6d'})) {
        return error({ statusCode: 404, message: 'Post not found' })
      }

      let response
      const version_id = _.get(_.find(introspection.activities, {id: 'eac4491c-9d01-474d-b5c0-3bfa095d2b6d'}), 'version_id', null)

      if (version_id) {
        response = await app.$api.activities.getVersionsByIds([version_id])
      } else {
        response = await app.$api.activities.getVersions({
          parent_ids: ['eac4491c-9d01-474d-b5c0-3bfa095d2b6d'],
          isDefault: true
        })
      }
      ...
      const selections_ids = matieres.map(selection => selection.id)
      ...
      response = await app.$api.activities.getUserLogs(app.store.state.auth.user.userID, 'eac4491c-9d01-474d-b5c0-3bfa095d2b6d')
      const activityLogs = response.data[0]

      if (activityLogs) {
        return { matieres, actions, activityLogs }
      } else {
        return { matieres, actions }
      }
    } catch ( e ) {
      return 'error'
    }
  ...
  data() {
    return {
      ...
      activityLogs: {
        status_name: 'open',
        activity_id: 'eac4491c-9d01-474d-b5c0-3bfa095d2b6d',
        config: {}
      },
      endResults: null
    }
  ...
  methods: {
    startActivity: function() {
      this.step = 1
      this.$router.push( 'matieres-preferees/etape-1' )
    },
