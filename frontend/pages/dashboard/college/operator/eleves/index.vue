  ...
  <div class="pt-16 px-5 mx-0 lg:px-0 xl:px-0 xl:pt-32 flex flex-col">
    ...
    <div class="flex flex-col xl:flex-row w-full justify-between items-center pb-10">
      <div class="flex flex-col xl:flex-row items-center">
        <div class="pb-2 xl:pb-0 ms-text text-ms-gray-light title-class-width"> 
          Classe{{ classes.length > 1 ? 's' : '' }} : 
        </div>
        <div v-if="classes.length > 1" class="flex flex-wrap">
    ...
    <Student v-for="student in students" :key="student.id" :student="student" :accountType="'operator'" class="my-3"/>
  ...
  export default {
    ...
    async asyncData ({app, store}) {
      try {
        ...
        let response = await app.$api.schools.getSchoolsByIds(user.config.school_ids)
        const school = response.data[0] || {}

        response = await app.$api.schools.getClassrooms({ school_ids: user.config.school_ids, page: 1, numberPerPage: 1000 })
    ...
    mounted() {
      if(this.$store.state.auth.user.config.accountType !== 'operator') {
