<template>
  <div class="pt-16 px-5 xl:px-0 xl:pt-32 flex flex-col">
    ...
    <FiltersPartner :account="account" :schools="schools" :gradesAll="grades" :typePage="'stats'" @getDataFilters="getDataStats" />
...
export default {
  ...
  name: 'DashboardFacilityStats',
  components: { FiltersPartner, BarChart },
  ...
  data() {
    return {
      ...
      textStudentStats: ['élèves suivis', 'établissements', 'classes', 'connexions'],
      options: {
        responsive: true,
  ...
  methods: {
    async getDataStats(dataFilters) {

      this.dataFilters = dataFilters

      try {
        let response = await this.$api.data.getDataStats({
